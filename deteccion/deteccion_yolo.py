...
def modo_rafaga():
    global rafaga_en_curso, disparo_activo
    while rafaga_en_curso:
        if esp:
            esp.write(b"DISPARO\n")
            print("[💥] Disparo activado (ráfaga)")
        disparo_activo = True
        time.sleep(0.3)

        if esp:
            esp.write(b"OFF\n")
            print("[⏸️] Pausa de disparo")
        disparo_activo = False
        time.sleep(1.5)
...
while True:
    ...
    if persona_objetivo:
        ...
        if ahora - tiempo_ultima_alerta > tiempo_entre_alertas and not sonando:
            if contador_alertas == 0:
                def aviso1():
                    global sonando
                    sonando = True
                    engine.say("Advertencia: Usted está ingresando a una zona restringida. Abandone el area.")
                    engine.runAndWait()
                    sonando = False
                ...
                sonando = True
                engine.say("Última advertencia, procediendo al ataque")
                engine.runAndWait()
                sonando = False
                contador_alertas += 1
                tiempo_ultima_alerta = ahora

                if puede_disparar and not rafaga_en_curso:
                    rafaga_en_curso = True
                    hilo_rafaga = threading.Thread(target=modo_rafaga, daemon=True)
                    hilo_rafaga.start()
        ...
        contador_alertas = 0
        sonando = False
        if disparo_activo or rafaga_en_curso:
            if esp:
                esp.write(b"OFF\n")
                print("[🛑] Disparo apagado")
            disparo_activo = False
            rafaga_en_curso = False

    if not puede_disparar and (time.time() - tiempo_ultimo_disparo >= espera_entre_disparos):
        puede_disparar = True
